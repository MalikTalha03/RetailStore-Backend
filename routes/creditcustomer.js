...
router.get("/:id", async (req, res) => {
  try {
    const creditcustomer = await CreditCustomerModel.find({
      id: req.params.id,
    });
...
router.delete("/:id", async (req, res) => {
  try {
    const creditcustomer = await CreditCustomerModel.findOneAndDelete({
      id: req.params.id,
    });
    res.json({
      message: `CreditCustomer ${creditcustomer.creditID} has been deleted`,
    });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
